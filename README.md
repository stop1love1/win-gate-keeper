# WinGateKeeper — User guide

WinGateKeeper helps you **set up a Windows PC or server** so other people can connect using **SFTP** (secure file transfer) or **SSH** (remote command-line login), with **separate accounts**, **separate folders**, and **activity logging**. You work in a **menu inside a dark window** (PowerShell): read the prompts on screen and type the number or letter it asks for, then press Enter.
...
## How to open WinGateKeeper (easiest way)

1. Open the folder that contains WinGateKeeper.
...
4. A new window opens — that is where you use the WinGateKeeper menu.
...
| **A** | **About / Author:** author **stop1love1** and link to the [GitHub repository](https://github.com/stop1love1/win-gate-keeper); optional open in browser. |
| **Q** | **Quit** the program. |
...
- The **main working folder** for WinGateKeeper.
...
## What is WinGateKeeper? (one line)
